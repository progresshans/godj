...
별도 구현·검증했습니다. EVID-097 docs head `c59669c...`는 run `32278555810`에서 닫혔고 D4e는
...
run `32282269755`에서 empty-source required Add를 별도 구현·검증했습니다. EVID-098 docs head `85f9270...`는
CI #94/run `32288383027`에서 닫혔고 D4f final head `9d5b894...`는
...
CI #95/run `32294983953`에서 bounded Remove-by-remake를 별도 구현·검증했습니다.
...
- Nullable/required relation Add/Remove authority는 whole-plan dry pass와 execution rematerialization에서 각각 재검증합니다.
  Public intent는 changed target 하나만 전달하고 SQLite는 그 immutable snapshot을 exact same symbolic target의
  pre-existing source relations에만 privately 확장합니다. Source model당 한 step의 relation mutation은 세
  capability를 합쳐 하나이며 resource node/byte cap을 allocation 전에 검사합니다. Required existing-source emptiness는 pinned
  `BEGIN IMMEDIATE` 뒤 physical preflight에서 claim 전에 확인하고 same-intent created source는 query 없이 statically
  empty로 취급합니다. Populated rejection은 rollback 뒤 claim/DDL/recorder 0입니다.
- D4f reverse/unapply 구현은 exact appended nullable `PROTECT` 또는 `SET_NULL`, required `PROTECT`만
  허용합니다. Frozen direct E2E fixture는 nullable `PROTECT`와 required `PROTECT`만 검증했으며 dedicated
  nullable `SET_NULL` D4f E2E proof는 주장하지 않습니다.
  `BEGIN IMMEDIATE` 뒤 pre-claim physical preflight는 remake source inbound FK/non-PK index, touched/control
  trigger/view, relevant generated/hidden/option, malformed/case-variant/noninteger/negative sequence와
  namespace/temp/control collision을 거부하지만 unrelated harmless object는 허용합니다. Claim 뒤
  count→temp create→retained-column PK-order copy→affected/
  stored count equality→drop/rename→sequence clear/restore/verify→final canonical/FK/`foreign_key_check`→recorder/
  revision→one commit이 같은 transaction/connection에서 실행됩니다.
...
`{true,true,false,false}`였습니다. D4d docs head `c59669c...`/run `32278555810`과 D4e final head
`1d86f6e...`/run `32282269755`는 각각 exact 26/26·342/342와 audit P0..P3=0을 통과했습니다. Required
empty-source preflight, native NOT NULL FK, fault rollback/no-retry와 reopen을 검증했고 capability는
당시 `{true,true,true,false}`였습니다. EVID-098 docs head `85f9270...`/CI #94 run `32288383027`과 D4f final
head `9d5b894...`/CI #95 run `32294983953`도 각각 exact 26/26·342/342와 audit P0..P3=0을 통과했습니다.
D4f mutation/final-FK fault는 original backward `AddField`를 소유하고 `errors.Is` cause, rollback once,
sticky no-retry와 no-temp-leak를 보존하며 recorder fault만 `NoOperation`입니다. Current capability는
`{true,true,true,true}`입니다. Raw-file equality/general restart/arbitrary remake는 주장하지 않습니다.
