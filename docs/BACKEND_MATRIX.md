...
SQLite의 literal empty/NUL 보정·빈 목록 확장은 ADR-0071/DEV-0017에 명시한다. Root JSON 경로 projection도 typed nullable 결과로 연결했다. 관계 filter source의 root scalar/JSON 경로 DTO는 같은 JOIN과 선택값 기준 DISTINCT를 사용한다. JSON range/order/Min/Max·relation path projection은 현재 미지원이다.
