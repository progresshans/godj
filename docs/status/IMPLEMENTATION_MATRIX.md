...
| Query | typed/dynamic 공통 AST, Boolean composition, scalar comparison·IN, 검증 뒤 빈 조회 SQL 생략, same-model F, projection·Count/Min/Max·관계 filter Count와 root DTO 선택 | JSON은 exact/IN/F exact·isnull·projection과 명시적 key/index 경로의 exact/IN/isnull, PostgreSQL contains/contained_by와 양 DB has_key/has_keys/has_any_keys(root/path·forward); root JSON path projection; SQLite containment·JSON range/order/Min/Max·relation path projection은 미지원; 넓은 reverse relation·tuple/subquery IN·annotation/grouping/window/bulk/locking은 별도 | [orm](../../orm/), [query](../../query/), [IN 의미](../adr/0062-scalar-membership-and-empty-query-execution.md) |
