...
| Auth/session | password hashing, Session/CSRF, injected strict Bearer verifier, rotation/logout; Credential snapshot·비밀번호 결합 stamp와 실제 HTTP 소비자 구현; 재사용 User/Group/Permission 저장과 native read snapshot의 Directory·비밀 표현을 제한한 불변 Account·직접/그룹 grant union; 관리자 password 교체의 현재 인가·revision·대상 session 폐기·transaction audit service, 환경별 검증은 Evidence 참조 | 저장 인증·role·operator adoption/첫 계정 생성·durable session/API·Article/Helpdesk 및 CLI 소비자의 영향 검증 완료; token issuer/JWT/OAuth/OIDC/self-service password/reset·나머지 multi-user 관리 service/UI/API는 미완료 | [auth](../../auth/), [sessions](../../sessions/) |
