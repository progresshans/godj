...
| SQLite/PostgreSQL | current AST/CRUD/migration/system-state paths; SQLite 새 physical connection의 FK 활성화/readback | 각 backend capability와 물리 schema precondition 적용; Decimal은 SQLite BLOB과 PostgreSQL NUMERIC의 exact 모델·DB 경로이며 Form/API 소비자는 진행 중; Float의 SQLite NaN 거부와 signed-zero 정규화, PostgreSQL native 특수값 의미 구분 | [Backend Matrix](../BACKEND_MATRIX.md) |
