...
| Migration | strict current definition, historical replay, immutable planner, revision fence, durable prefix, choices-only AlterField와 bounded reverse; historical self/cyclic graph·여러 FK Add/Remove·transitive target; self/later/cyclic 자동 계획·선언 순서와 prefix 재개 | required FK 추가는 빈 table에서만 적용; 넓은 schema 변경·backfill은 후속 | [migrations](../../migrations/) |
