...
| Auth/session | password hashing, Session/CSRF, injected strict Bearer verifier, rotation/logout; Credential snapshot·비밀번호 결합 stamp와 실제 HTTP 소비자 구현, 환경별 검증은 Evidence 참조 | token issuer/JWT/OAuth/OIDC/password reset·multi-user lifecycle 별도 | [auth](../../auth/), [sessions](../../sessions/) |
