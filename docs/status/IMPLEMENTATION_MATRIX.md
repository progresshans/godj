...
| 관계 | AutoField-target FK, 자기·상호 참조 선언/생성, lazy/forward/reverse, 최대 64 hop forward의 scalar lookup·FK isnull과 AND/OR/NOT, prefetch/eager All·First·Count와 여러 direct forward selected 관계·다른 filter JOIN 조합, assignment/cache, PROTECT/SET_NULL | eager First는 명시적 정렬 필요; reverse non-exact/OR/NOT·relation F·무제한 깊이·nested/reverse eager projection·일반 순환 관계 동작·ManyToMany/OneToOne 미지원 | [관계 소유권](../CONCURRENCY.md#관계-객체) |
