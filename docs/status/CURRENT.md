...
- 갱신: 2026-09-19
- 활성 작업: [GDJ-0077 관계 조회의 Count와 캐시 의미](../../work/0077-eager-count-and-query-cache-semantics.md)
- 최근 완료: [GDJ-0076 모델 선택값과 메타데이터 변경 이력](../../work/0076-model-choices-and-metadata-migrations.md)
...
String/int64 choices를 모델·Form/Admin/API에 연결하고 물리 DDL 없는 historical AlterField로 변경 이력을 보존한다. 현재 지원 범위는 [구현 현황](IMPLEMENTATION_MATRIX.md)을 따른다.
...
GDJ-0077의 관계 조회 Count를 별도 checkout에서 공통 runtime·generated API·독립 소비자에 연결하고 영향 범위를 검증한다.
GDJ-0076은 외부 adapter fixture 누락 수정 뒤 로컬·Hosted ORM 검증을 완료했다. 정확한 source와 scope는 TEST_EVIDENCE를 따른다.
