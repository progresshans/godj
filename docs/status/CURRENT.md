...
Owner별 slice의 독립 기준과 Query AST·양 DB window compiler를 연결했다.
Slice snapshot의 runtime/generated 접근자 연결은 다음 범위다.
...
일반 manager와 분리된 slice snapshot·owner batch 경계·나머지 target query 구성을 완성하고 Ticket 라벨 컬렉션 편집으로 이어간다.
