...
- 상태: 로컬 checkpoint 완료. 고정 source의 관련 Hosted 검증은 후속 기록이 소유한다.
...
로컬 Docker daemon이 실행 중이지 않아 PostgreSQL sentinel은 로컬에서 skip됐다. PostgreSQL 실행 결과는 Hosted 기록 전까지 미검증이다.
