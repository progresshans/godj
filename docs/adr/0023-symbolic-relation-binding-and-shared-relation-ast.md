...
> 당시의 전체 기록과 실행 증거는 [고정 원문](https://github.com/progresshans/godj/blob/003afee4524a0294ada8f02c140781f3e1751a5c/docs/adr/0023-symbolic-relation-binding-and-shared-relation-ast.md)에 있다.

- 상태: Accepted
- 날짜: 2026-08-10
- 관련 work/contract:
  [GDJ-0023](https://github.com/progresshans/godj/blob/003afee4524a0294ada8f02c140781f3e1751a5c/work/0023-foreign-key-relation-compatibility-contracts-and-binding-feasibility.md),
...
[EVID-20260810-032](https://github.com/progresshans/godj/blob/003afee4524a0294ada8f02c140781f3e1751a5c/docs/status/TEST_EVIDENCE.md#evid-20260810-032--gdj-0023-github-hosted-exact-22-job-implementation-head-ci)를
...
초기 field-union/split-IR와 별도 binder/AST feasibility 구현은 GDJ-0057에서 제거했다.
당시 실험과 선택 근거는 위 고정 원문 및 [실험 Git 트리](https://github.com/progresshans/godj/tree/da1bfc524c4f205075fc7fac7f00b437473a5e1f/conformance/relationbinding)에 남아 있다.
이미 채택된 설계를 별도 모형으로 다시 구현해 매번 플랫폼 검증할 필요는 없다.

현재 `schema/ir`, `orm`의 actual project binder, typed/dynamic relation AST, `codegen`의 상호·자기 참조 external module과
`conformance/relation*product`의 실제 DB/transaction 관찰이 같은 위험을 소유한다.
실패 시 partial binding/output을 게시하지 않는 것, app-to-app import cycle 방지, join/nullability/prefetch 의미와
SET_NULL rollback은 현행 제품의 부정 회귀로 검증한다. 현재 지원 범위는 [Matrix](../status/IMPLEMENTATION_MATRIX.md)를 따른다.
