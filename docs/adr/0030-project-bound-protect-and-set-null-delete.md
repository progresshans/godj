...
당시 classification이 exact `121 passing + 5 deviation + 1 oracle_locked`, relation 11/12였습니다. REL-002는
그 시점에 그대로 locked였습니다.

ADR-0035는 이 ADR의 project-bound incoming policy, pinned-transaction `PROTECT`/`SET_NULL`과 오류 의미를
유지합니다. 다만 본문의 IR v2/v3 target 분기, additive generated companion, 기존 generated bytes 보존과
publication topology는 pre-reset historical implementation evidence이며 current ABI 정본이 아닙니다.
