...
| Q-013 | Partial | GDJ-0032 active / relation facade 후속 | Accepted relation semantics와 bounded REL-007/008 low-level delete; production forward facade first-publication을 검토 중이고 reverse·FK mutation/cache policy는 open |
...
| Q-017 | P1 | GDJ-0032 active / API decision gate 전 | production relation-aware project facade의 첫 공개 경계와 이후 generated code upgrade 정책은 무엇인가 |
...
additive first-publish하는 bounded product packet입니다. 이 activation 시점에도 public 이름과 generated bytes는
noncanonical이며 Q-017은 open입니다.
...
GDJ-0032가 고정하는 product 경계는 다음과 같습니다.
...
보지 않습니다. `project.Using`, returned `Models`, `BlogPosts`, `Related`, `First` tuple, `Model` unwrap와 selector
이름은 모두 noncanonical이며 후속 Q-017 work가 별도로 결정합니다.

GDJ-0032는 exact 이름·selector representation·normalized naming/collision을 구현 전 decision gate로 남깁니다.
Binder-first, original binder-error precedence, valid binding 뒤 nil/typed-nil backend의
`backend_error/invalid_plan`과 I/O 0은 activation 계약으로 이미 고정하며 detail message만 noncontractual입니다. 새
companion 한 파일의 first publication은 general multi-file upgrade, CLI, rename/deprecation/repair 정책의 답이
아닙니다. 따라서 production forward facade가 완료돼도 broader generated upgrade 질문은 Q-017에 남을 수 있습니다.
