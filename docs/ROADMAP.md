...
[헌장](CHARTER.md)의 장기 제품 범위를 목표로, 필요한 기반과 기능 사이의 의존성에 따라 구현 순서를 정한다.
문서·테스트 개수나 compatibility contract 총합을 제품 진행률로 삼지 않는다.

## 완성 목표와 작업의 연결

목표는 [기능 카탈로그](CAPABILITY_CATALOG.md)의 전체 범위를 실제로 사용할 수 있게 구현하고 검증하는 것이다.
우선 모델 타입·관계·query·migration의 공통 의미를 넓히고, 이를 Form/Admin/API와 identity의 실제 흐름에 연결한다.
그 기반을 사용해 나머지 Web/contrib·추가 backend·Realtime·GIS·지역화를 확장한다. 구체적인 의존성이 드러나면 순서를 조정한다.
현재 예제에 없는 요구도 카탈로그의 미완료 범위로 계속 관리하며, 한 예제의 성공만으로 전체 영역을 완료 처리하지 않는다.

각 범위는 공개 API와 실패 의미, 실제 소비자, 관련 환경의 실행 근거를 갖춰야 완료로 기록한다.
남은 기능이나 필수 검증 환경을 미지원·미검증으로 남겨 둔 상태에서 전체 완성을 선언하지 않는다.
...
작업을 나누는 것은 구현과 검증을 관리하기 위한 방식이며, 각 작업의 범위가 전체 제품의 목표 범위를 제한하지 않는다.
...
현재는 출시 일정을 두지 않고 목표한 기능과 기반의 구현·검증을 계속한다.
출시 여부와 시점, 외부 지원 정책은 사용자가 출시를 결정하는 단계에서 별도로 정한다.
