...
func TestTimeHistoricalDefaultsRoundTripAndRejectNoncanonicalValues(t *testing.T) {
	for _, instant := range []clock.Time{{}, {Hour: 12, Minute: 34, Second: 56, Microsecond: 123456}, {Hour: 23, Minute: 59, Second: 59, Microsecond: 999999}} {
		model, err := schema.Build(schema.Definition{AppLabel: "events", Models: []schema.Model{{Name: "event", GoName: "Event", Fields: []schema.Field{schema.TimeField("at", "At", schema.Nullable(), schema.Default(instant))}}}})
		if err != nil {
			t.Fatal(err)
		}
		...
		canonical := model.Models[0].Fields[1].Default.Time
