...
func collectFieldCandidates(value jsonValue, sourceID, pointer, app, name string, operationIndex int) []failureCandidate {
	...
	if stringValid["kind"] {
		...
		case ir.FieldBoolean, ir.FieldInteger, ir.FieldDateTime, ir.FieldDate, ir.FieldTime, ir.FieldDuration, ir.FieldFloat, ir.FieldDecimal, ir.FieldUUID, ir.FieldJSON:
			...
			if ir.FieldKind(kind.string) == ir.FieldJSON {
				expectedDefault = ir.ScalarJSON
			}
...
func collectDefaultCandidates(value jsonValue, sourceID, pointer, app, name string, operationIndex int) []failureCandidate {
	...
	switch kind.string {
	case string(ir.ScalarJSON):
		object, _, faults := semanticObjectCandidates(value, []string{"json", "kind"}, sourceID, pointer, app, name, operationIndex, CodeInvalidIR)
		candidates = append(candidates, faults...)
		if child, present := object.member("json"); present {
	...
	case string(ir.ScalarFloat):
		...
		candidates = append(candidates, faults...)
		if child, present := object.member("float_bits"); present {
			_, err := floatvalue.FromBits(child.string)
			if child.kind != jsonString || err != nil {
				candidates = append(candidates, semanticFailure(CodeInvalidIR, sourceID, pointer+"/float_bits", app, name, operationIndex, "invalid_ir"))
			}
		}
	...
	case string(ir.ScalarTime):
		...
		candidates = append(candidates, faults...)
		if child, present := object.member("time"); present {
			_, err := clock.Parse(child.string)
			if child.kind != jsonString || err != nil {
				candidates = append(candidates, semanticFailure(CodeInvalidIR, sourceID, pointer+"/time", app, name, operationIndex, "invalid_ir"))
			}
		}
...
func materializeDefault(value jsonValue) (*ir.Scalar, bool) {
	...
	switch kind.string {
	case string(ir.ScalarJSON):
		payload, exists := value.member("json")
		if !exists || payload.kind != jsonString {
			return nil, false
		}
		parsed, err := jsonvalue.Parse([]byte(payload.string))
		if err != nil || parsed.Text != payload.string {
			return nil, false
		}
	...
	case string(ir.ScalarFloat):
		...
		if _, err := floatvalue.FromBits(payload.string); err != nil {
			return nil, false
		}
		return &ir.Scalar{Kind: ir.ScalarFloat, FloatBits: payload.string}, true
	case string(ir.ScalarDuration):
	...
	case string(ir.ScalarTime):
		...
		if _, err := clock.Parse(payload.string); err != nil {
			return nil, false
		}
		return &ir.Scalar{Kind: ir.ScalarTime, Time: payload.string}, true
	case string(ir.ScalarDate):
