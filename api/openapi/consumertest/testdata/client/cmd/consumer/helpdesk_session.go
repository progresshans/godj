...
func checkHelpdeskSession(ctx context.Context, target endpoint) error {
	...
	if err != nil || len(initial) != 1 || initial[0].ID != target.TicketID || initial[0].Category != target.CategoryID || initial[0].Subject != "Existing ticket" || !initial[0].Priority.Null || !initial[0].Resolution.Null || !initial[0].DueAt.Null || !initial[0].Reviewed.Null || !initial[0].ServiceOn.Null || !initial[0].ServiceAt.Null || !initial[0].Elapsed.Null || !initial[0].Effort.Null || !initial[0].ExpectedCost.Null {
	...
	for _, test := range []struct {
		...
		if !value.ServiceAt.Null || !value.Elapsed.Null || !value.Effort.Null || !value.ExpectedCost.Null || value.ServiceOn.Null != (!day.Set || day.Null) || (!value.ServiceOn.Null && !value.ServiceOn.Value.Equal(day.Value)) {
