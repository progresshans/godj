...
func TestGeneratedJSONConsumer(t *testing.T) {
	...
	writeGeneratedTestFile(t, root, "consumer/json_comparison_test.go", comparison)
	for _, item := range []struct{ source, target string }{
		{"godj-canonical-comparison-sqlite.json", "comparison_sqlite_reference.json"},
		{"django61-comparison-postgres.json", "comparison_postgres_reference.json"},
	} {
