...
func TestGeneratedMultipleEagerConsumer(t *testing.T) {
	s, err := schema.Build(schema.Definition{AppLabel: "join_reference", Models: []schema.Model{
		...
		{Name: "team", GoName: "Team", Fields: []schema.Field{schema.CharField("label", "Label", 40)}},
		{Name: "post", GoName: "Post", Fields: []schema.Field{schema.CharField("title", "Title", 40), schema.ForeignKey("author", "AuthorID", schema.Target("join_reference", "person"), schema.RelatedName("posts"), schema.Protect), schema.ForeignKey("reviewer", "ReviewerID", schema.Target("join_reference", "person"), schema.RelatedName("reviews"), schema.SetNull, schema.Nullable()), schema.ForeignKey("team", "TeamID", schema.Target("join_reference", "team"), schema.RelatedName("posts"), schema.SetNull, schema.Nullable())}},
	...
	const module = "example.com/godj-multiple-eager"
	...
	consumer, err := os.ReadFile("testdata/multipleeager/consumer_test.go")
	if err != nil {
		t.Fatal(err)
	}
	writeGeneratedTestFile(t, root, "consumer/consumer_test.go", consumer)
	reference, err := os.ReadFile(filepath.Join(codegenRepositoryRoot(t), "orm/testdata/multiple-eager-django61.json"))
	if err != nil {
		t.Fatal(err)
	}
	...
	assertGeneratedConsumerTests(t, runStrictGeneratedCommand(t, command), "TestGeneratedMultipleEagerReference", "TestGeneratedMultipleEagerOwnershipAndDerivation", "TestGeneratedMultipleEagerInvalidSelectors")
}
