...
const (
	GeneratorVersion         = "godj-codegen-m2-v3"
	RelationGeneratorVersion = "godj-codegen-rel-v1"
)
...
func Generate(packageName string, input ir.Schema) ([]byte, error) {
	...
	if schema.FormatVersion == ir.RelationFormatVersion {
		if err := validateRelationGeneratedNames(schema); err != nil {
		...
		return generateRelationMain(packageName, schema, hash)
	}
...
func generateRelationMain(packageName string, schema ir.Schema, hash string) ([]byte, error) {
	var output bytes.Buffer
	fmt.Fprintln(&output, "// Code generated by GoDj. DO NOT EDIT.")
	fmt.Fprintln(&output)
	fmt.Fprintf(&output, "package %s\n\n", packageName)
	...
	formatted, err := format.Source(output.Bytes())
	if err != nil {
		return nil, fmt.Errorf("format generated relation source: %w", err)
	}
	return formatted, nil
}
...
func validateRelationGeneratedNames(schema ir.Schema) error {
	...
	for _, model := range schema.Models {
		if previous, exists := packageSymbols[model.GoName]; exists {
			return fmt.Errorf("package symbol %s for model %s conflicts with %s", model.GoName, model.GoName, previous)
		}
		packageSymbols[model.GoName] = "model " + model.GoName
	}
	return nil
}
