	...
	case ResultAggregate:
		...
		for _, expression := range s.expressions {
			...
			case ResultMax, ResultMin:
				field, ok := expression.Field()
				if !ok || !validResultField(field) || (field.Kind() != FieldInteger && field.Kind() != FieldString && field.Kind() != FieldDateTime && field.Kind() != FieldDate && (field.Kind() != FieldTime && field.Kind() != FieldDuration)) {
