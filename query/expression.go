...
func validateExpressionCondition(condition Condition) error {
	...
	case conditionRHSJSONKeys:
		...
		if condition.relationPath != nil && !targetMembershipPath(*condition.relationPath) {
			return invalidPlanError("related JSON key presence requires a valid target-field path with collection row identity")
		}
	...
	case conditionRHSList:
		...
		if (condition.relationPath != nil && !targetMembershipPath(*condition.relationPath)) || !validInValues(field, condition.rhs.values) {
