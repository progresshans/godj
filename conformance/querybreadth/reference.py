"""Generate or verify the pinned QRY-022..033 Django SQLite oracle."""
...
def generate_suite(
    ...
    if profile.get("format_version") != 2 or manifest.get("format_version") != 2:
        raise RuntimeError("query-breadth profile and manifest must use format_version 2")
    if manifest.get("profile_id") != profile.get("id"):
        raise RuntimeError("query-breadth manifest profile_id mismatch")

    contracts = manifest.get("contracts")
    if not isinstance(contracts, list) or len(contracts) != 12:
        raise RuntimeError("query-breadth manifest must contain exactly 12 contracts")
    expected_ids = [f"QRY-{number:03d}" for number in range(22, 34)]
    if [contract.get("id") for contract in contracts] != expected_ids:
        raise RuntimeError("query-breadth manifest contract order mismatch")
    if [contract.get("scenario") for contract in contracts] != list(
        scenarios.SCENARIOS
    ):
        raise RuntimeError("query-breadth manifest scenario order mismatch")
    if any(
        contract.get("status") not in ORACLE_READY_STATUSES
        for contract in contracts
    ):
        raise RuntimeError(
            "query-breadth oracle generation requires oracle_locked or passing status"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"query-breadth reference failed: {error}", file=sys.stderr)
        return 2
    return 0
