...
func TestReferenceOracleChecksumCatalogMatchesCurrentArtifacts(t *testing.T) {
	...
	if string(contents) != previous+definitionSource+projectCheck+migrationCommand+migrationWriter+migrationStatus+migrationTargetPlan+relation+migrationRelation+queryExpression+gdj0043+systemState {
...
func TestRelationProductEntersTwelveAdapterTargetWithoutChangingReferenceTargets(t *testing.T) {
	...
	if got := strings.Count(productTarget, "go run ./conformance/cmd/godjcheck"); got != 24 {
		t.Fatalf("godj-conformance adapter count = %d, want 24", got)
	}
