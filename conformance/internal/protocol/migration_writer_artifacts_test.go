...
func TestMigrationWriterArtifactsAreLockedValidatedAndPayloadFree(t *testing.T) {
	t.Parallel()
	...
	for name, want := range map[string]artifactLock{
		"conformance/contracts/migration-writer-manifest.json": {
			size:   8864,
			sha256: "75b3f485d392dceb5800b68efe267e6cb7010d873f845ecc0fa7c459b00f5d1e",
		},
	...
	for index, contract := range manifest.Contracts {
		...
		if contract.ID != wantID || contract.Scenario != migrationWriterScenarios[index] || contract.Phase != migrationWriterPhases[index] || contract.Status != ContractOracleLocked || !reflect.DeepEqual(contract.Comparison, wantComparisons) {
...
func TestMigrationWriterAuthorityAndCentralWiringRemainSeparated(t *testing.T) {
	...
	for _, fragment := range []string{"MIGRATION_WRITER_MANIFEST", "MIGRATION_WRITER_ORACLE", "MIGRATION_WRITER_NOT_IMPLEMENTED"} {
		if strings.Contains(productTarget, fragment) {
			t.Fatalf("Phase A product adapter unexpectedly references %s", fragment)
		}
	}
	if got := strings.Count(productTarget, "go run ./conformance/cmd/godjcheck"); got != 22 {
		t.Fatalf("product adapter count = %d, want preserved 22", got)
	}
...
func assertMigrationWriterProvenance(t *testing.T, contract Contract, djangoAuthority bool) {
	...
	for index, want := range []struct{ kind, reference string }{{"proposal", "GDJ-0050"}, {"decision", "ADR-0052"}} {
