...
func TestMigrationProjectCheckWorkflowExpandsToExactTwentySevenRequiredExecutions(t *testing.T) {
	...
	for _, required := range []string{
		...
		`payload = b"".join(`,
		`assert len(runs) == 950`,
		`assert len(payload) == 97469`,
		`7c1546c3c98179bba31e8bd27cf28d7e4f751897aaaa31c03bac76f0e7ddac29`,
		`assert passes == runs`,
		`assert skipped == [], skipped`,
	...
	for _, required := range []string{
		...
		"len(SCENARIOS) == 161",
		"len(payload) == 702415",
		"aa0d321264e0ad9eed1818d1530a51d18592c16d509c51417e4bdf598655b10e",
		"git diff --exit-code",
		`test -z "$(git status --porcelain=v1)"`,
	} {
