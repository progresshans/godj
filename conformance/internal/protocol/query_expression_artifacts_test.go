...
func TestQueryExpressionReferenceAndProductWiringIsLocked(t *testing.T) {
	...
	if got := strings.Count(productTarget, "go run ./conformance/cmd/godjcheck"); got != 21 {
		t.Fatalf("godj-conformance adapter count = %d, want 21 with query-expression included", got)
	}
	...
	if got := strings.Count(oracleCheckTarget, "python -m conformance.runners.django"); got != 22 {
		t.Fatalf("oracle-check reference runner count = %d, want 22", got)
	}
