...
func TestCheckedInOracleChecksumsMatchArtifacts(t *testing.T) {
	...
	wantedPaths := []string{"migration-execution-oracle.json", "migration-lifecycle-oracle.json", "migration-planning-oracle.json", "migration-restart-oracle.json", "migration-state-reconstruction-oracle.json", "oracle.json", "query-cache-oracle.json", "save-lifecycle-oracle.json", "write-migration-oracle.json", "migration-definition-source-oracle.json", "migration-project-check-oracle.json"}
...
func TestTenSetProductStatusesAre100PassingAnd5ReviewedDeviations(t *testing.T) {
	t.Parallel()
	...
	if passing != 100 || deviations != 5 {
		t.Fatalf("ten-set product statuses = %d passing + %d deviation, want 100 + 5", passing, deviations)
	}
}
