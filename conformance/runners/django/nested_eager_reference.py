"""Independent Django 6.1 nested forward eager graph observations, BSD-3-Clause."""
import json
...
selections=[
 ["author__team__organization"],
 ["reviewer__team__organization"],
 ["reviewer__backup__organization"],
 ["author__manager__manager"],
 ["author__team__parent__parent__parent"],
 ["author","author__team__organization","reviewer__backup__organization"],
 ["author__team","author__team__organization","author__backup"],
 ["reviewer__manager__team__organization","reviewer__team__organization"],
]
def snapshot(value):
    if value is None:return None
    result={}
    for field in value._meta.concrete_fields:
        item=getattr(value,field.attname)
        if isinstance(item,datetime):item=item.isoformat()
        result[field.attname]=item
    return result
def graph(row,selected):
    result={"source":snapshot(row),"targets":{}}
    for path in selected:
        current=row;prefix=[]
        for name in path.split("__"):
            prefix.append(name)
            current=None if current is None else getattr(current,name)
            result["targets"]["__".join(prefix)]=snapshot(current)
    return result
observations=[]
for selected in selections:
    for name,expr in cases:
        ...
        if name.startswith("reverse"):
            variants += [("distinct",True,0,None),("slice",False,1,3)]
        elif name in ("reviewer_name_null","two_routes_or","reviewer_empty"):
            variants += [("zero",False,0,0)]
        for suffix,distinct,offset,limit in variants:
            query=Post.objects.filter(build(expr)).order_by("id").select_related(*selected)
            ...
            with CaptureQueriesContext(connection) as first_sql:
                first=query.first();first=None if first is None else graph(first,selected)
            with CaptureQueriesContext(connection) as all_sql:
                values=list(query);rows=[graph(row,selected) for row in values]
            with CaptureQueriesContext(connection) as warm_sql:
                warm_count=query.count();warm_first=query.first();warm_first=None if warm_first is None else graph(warm_first,selected)
                warm_rows=[graph(row,selected) for row in query]
            observations.append({"name":"|".join(selected)+"/"+name+"/"+suffix,"expression":expr,"selected":selected,"distinct":distinct,"offset":offset,"limit":limit,"count":count,"first":first,"rows":rows,"warm_count":warm_count,"warm_first":warm_first,"warm_rows":warm_rows,"count_sql":[r["sql"] for r in count_sql],"first_sql":[r["sql"] for r in first_sql],"all_sql":[r["sql"] for r in all_sql],"warm_queries":len(warm_sql)})
