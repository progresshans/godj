"""Independent Django 6.1 multiple selected relation observations (BSD-3-Clause).
...
class Post(models.Model):
    ...
    team = models.ForeignKey(Team, on_delete=models.SET_NULL, related_name='posts', null=True)
...
with connection.schema_editor() as editor:
    for model in (Person, Team, Post, Comment):
        editor.create_model(model)
teams = [Team.objects.create(label='Red'), Team.objects.create(label='Blue')]
...
for title, author, reviewer in [('keep', 1, None), ('drop', 2, None), ('keep', 1, 1), ('drop', 2, 1), ('keep', 1, 2), ('drop', 2, 2), ('keep', 3, 3)]:
    Post.objects.create(title=title, author_id=author, reviewer_id=reviewer, team_id=1 if reviewer is None else 2 if reviewer == 2 else None)
...
def data(row, selected):
    ...
    for name in dict.fromkeys(selected):
        ...
        if target is None:
            ...
            targets[name] = {'id': target.pk, 'label': target.label}
        else:
            targets[name] = {'id': target.pk, 'name': target.name, 'nickname': target.nickname, 'active': target.active}
    return {'id': row.pk, 'targets': targets}
observations = []
for selected in (('author', 'reviewer'), ('reviewer', 'author'), ('author', 'reviewer', 'team'), ('team', 'reviewer'), ('author', 'author', 'reviewer')):
    for name, expr in cases:
        ...
        for suffix, distinct, offset, limit in variants:
            ...
            observations.append({'name': f"{'_'.join(selected)}_{name}_{suffix}", 'selected': selected, 'expression': expr, 'distinct': distinct, 'offset': offset, 'limit': limit, 'count': count, 'first': first, 'rows': rows, 'warm_count': warm_count, 'warm_first': warm_first, 'count_sql': [q['sql'] for q in count_sql], 'first_sql': [q['sql'] for q in first_sql], 'all_sql': [q['sql'] for q in all_sql], 'warm_queries': len(warm_sql)})
failures=[]
for selected in [("author","missing"),("author","title"),("author","comments"),("author","reviewer__missing")]:
    for limit in [None,0]:
        terminals={}
        for action in ["count","first","all"]:
            qs=Post.objects.select_related(*selected).order_by("id")
            if limit is not None: qs=qs[:limit]
            with CaptureQueriesContext(connection) as captured:
                try:
                    value=qs.count() if action=="count" else qs.first() if action=="first" else list(qs)
                    terminals[action]={"error":None,"empty":value is None or value==[] or value==0}
                except Exception as error:
                    terminals[action]={"error":type(error).__name__}
            terminals[action]["queries"]=len(captured)
        failures.append({"selected":selected,"limit":limit,"observations":terminals})
print(json.dumps({'django': django.get_version(), 'leaves': leaves, 'observations': observations, 'invalid_selection_observations':failures}, indent=2))
