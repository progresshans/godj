...
Add를 구현·검증했습니다. Populated required Add, Remove/remake, general restart와 MIG product adapter/status
전환은 아직 완료하지 않았습니다.
...
- D3a 당시 SQLite capability는 CreateModel FK만 true였습니다. D4d 이후 tuple은
  ...
  `{CreateModelForeignKeys:true, AddNullableForeignKey:true,
  AddRequiredForeignKeyToEmptyTable:true, RemoveForeignKeyByTableRemake:false}`입니다. Relation Add는 public
  ...
  required relation Add를 합쳐 하나입니다. Required는 no-default/non-PK/`PROTECT`와 empty source만 허용하며
...
- [x] EVID-097 docs head `c59669c...`와 bounded `AddRequiredForeignKeyToEmptyTable` final head `1d86f6e...`의
      ...
      audit P0..P3=0
- [ ] 그 다음 `RemoveForeignKeyByTableRemake`, actual adapter, overall
...
다음 정확한 작업은 D4f Remove/remake입니다. 그 뒤 actual adapter → overall completion → terminal 순서로 닫습니다.
...
현재 capability는 `{true,true,true,false}`이고 empty-source required Add만 true입니다. Populated required Add와
Remove/remake, general restart/actual adapter는 미지원입니다. Reference는 exact
...
`1d86f6e...`만 증명합니다. 이 EVID-098 documentation mirror, later D4f/adapter, completion/terminal 또는 MIG
status 전환을 재귀적으로 증명하지 않습니다. Public raw
