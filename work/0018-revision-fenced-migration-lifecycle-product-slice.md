...
- 실제 `Executor.Backend`를 재사용하는 `Executor.Migrate(ctx, definitions, request)` 후보 검증
...
[Proposed ADR-0018](../docs/adr/0018-revision-fenced-migration-lifecycle-product-shape.md)의
검증 후보는 다음입니다.
...
- Definition과 nested operation/IR은 호출 초기에 snapshot/deep-copy하며 caller mutation이 진행
...
- Session open/snapshot/begin/commit/rollback/close/cancellation resource가 누수되지 않는지는
  SQLite connection count, race와 fault gate로 검증합니다. 이 exact shape도 Proposed이며
  implementation evidence가 반박하면 ADR을 Accepted하기 전에 수정합니다.

## SQLite storage와 adoption 경계

Metadata v1 후보는 recorder와 별도 singleton table/row에 다음을 저장합니다.
...
Public `migrations.Error`에 다음 category/code 후보를 검증합니다.
...
1. Proposed ADR-0018의 public request, `Executor.Migrate`와 optional session port를 external
   compile test로 spike하고 zero/nil/copy/error ownership을 확정합니다.
...
- [ ] Tagged latest/targeted request와 invalid zero semantics
- [ ] `Executor.Migrate(ctx, definitions, request)` external compile/public ownership 확정
- [ ] Atomic identities+private revision snapshot과 explicit history preflight
- [ ] Existing reconstructor/planner/executor kernel 재사용과 definition deep-copy
- [ ] Per-step first-write fence, atomic successor와 committed/rolled-back/unknown state 의미
- [ ] Exact-one session snapshot, mandatory Close와 active/poisoned/closed lifetime/resource gate
- [ ] Fresh bootstrap, empty no-op와 existing recorder adoption-required
- [ ] Metadata 뒤 legacy writer fail-closed와 unsupported backend no fallback
- [ ] Stale/contention/capability/integrity/commit_outcome_unknown structured error와 semantic retry 0
- [ ] Two connection/process single winner, between-step conflict와 ABA/fault/cancellation gate
- [ ] Empty-table default AddField logical default/physical no-default와 nonempty 거부
- [ ] MIG-047..056 GoDj live adapter, two-process actual byte identity와 9 exact + DEV-0002 expectation
- [ ] DEV-0002 decision/status/evidence와 six-path sparse scope가 DEVIATIONS/policy/fixture에서 일치
- [ ] MIG-052 status + exact-one provenance/expectation 외 locked lifecycle oracle/static/SHA256SUMS 불변
- [ ] `conformance/lifecyclefence/**` byte 불변과 existing public port fake compile 회귀
- [ ] Existing `83 passing + 4 deviation` 회귀 없이 `92 passing + 5 deviation`
- [ ] Nine product adapter, 97 global unique IDs/scenarios와 72 cross-binding 유지
- [ ] Full Go/race/CGO=0/vet, portable/exact Python, compile/source/mutation gate
- [ ] 독립 P0–P3 product/conformance/fence audit
...
- [ ] Public API/backend session compile spike
- [ ] Core/SQLite product 구현
- [ ] GoDj lifecycle adapter와 status 전환
- [ ] Full 검증·독립 감사·문서 handoff

## 수정 파일

Activation은 이 work, Proposed ADR-0018, work/ADR index, CURRENT, ROADMAP과
OPEN_QUESTIONS만 변경합니다. 제품 구현 뒤에는 실제 변경 파일, manifest hash와 adapter
ownership을 이 절에 기록합니다.

## 결정된 사항

- 2026-08-08: GDJ-0018은 source loader보다 already-loaded definition lifecycle 제품을 먼저
  구현합니다.
- 2026-08-08: Actual `Executor`가 backend field를 소유하므로 중복 backend 인자 없는
  `Executor.Migrate`를 우선 API 후보로 검증합니다.
...
- 2026-08-08: Oracle/static/SHA256SUMS와 test-only lifecyclefence spike는 제품 work에서
  수정하지 않습니다.
- 2026-08-08: Accepted ADR-0013의 canonical ascending order를 보존합니다. MIG-052의
  incomparable B1/A3 sibling order만 DEV-0002 sparse expectation 후보로 분리하고 final
  state/schema/history는 Django와 동일해야 합니다.
- 2026-08-08: Fenced lifecycle은 legacy `Transaction`이 아니라 dedicated
  `RevisionFencedTransaction`, explicit commit durability와 mandatory session `Close` 후보를
  사용합니다. Session은 call 사이 connection-free이고 SQLite step만 pinned connection을 소유합니다.

## 미결정/Blocker

외부 blocker는 없습니다. 다음 항목은 구현/compile/fault spike 결과로 ADR-0018을 Accepted하기
전에 확정합니다.

- Backend raw failure carrier와 public `migrations.Error` mapping의 최소 source-compatible shape
- Metadata table/column의 exact identifier와 corrupt/partial object diagnostics
- `CommitOutcome` carrier에 durability 외 cleanup detail을 더 노출할 필요가 있는지

Public adoption API, restore/overflow/crash repair는 이 work의 blocker가 아니라 명시적 비목표입니다.
...
- Product evidence ID: 구현 완료 시 새 append-only entry 할당
- Activation validation: Markdown frontmatter/local links와 `git diff --check`
- Not run at activation: 제품 source 변경이 없으므로 Go/Python product gate는 구현 commit에서 실행

## 위험과 rollback

- Public API/metadata v1을 검증 전 Accepted로 올리지 않습니다.
...
`migrations/backend/lifecycle.go`와 `migrations/lifecycle.go`의 compile-only 최소 shape를 먼저
작성하고 external fake가 기존 port만 구현해도 계속 compile되는지 확인합니다. 이어서 fake
revision session으로 zero/latest/target/history/last-durable unit test를 먼저 통과한 후 SQLite
metadata를 구현합니다.

## 결과와 인수인계

현재는 activation 단계입니다. 제품 분류는 여전히 `83 passing + 4 deviation + 10
oracle_locked`이고 Proposed ADR-0018/DEV-0002는 구현/검증 전 후보입니다. 완료 목표는
`92 passing + 5 deviation`입니다. 완료 뒤 다음 작업은
GDJ-0019 migration definition source/versioned loader contract이며, GDJ-0018 API가 file layout,
operation codec나 CLI handshake를 미리 고정하지 않도록 유지합니다.
