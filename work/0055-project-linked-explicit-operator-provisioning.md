...
- Logical failure stdout은 zero이고 stderr에는 이미 게시된 prompt 뒤 stable `category/code\n`만 한 번 씁니다.
...
- Raw child stderr, hasher/backend cause, username/password/hash, DB URL/path와 partial private bytes는 public error/unwrap/protocol/report에
  포함하지 않습니다.
- Success publication short-write/error는 durable create를 되돌리거나 retry하지 않으며 known-created output failure로 닫습니다.
...
| SYS-026 | rollback/commit unknown no-retry와 known-created cleanup/output outcome 보존 |
...
- [ ] Phase A — exact decision/reference manifest, not-implemented artifact와 protocol/false-green lock
- [ ] Phase B — systemstate provision/open-existing split, atomic/error tests와 current-only callsite refreeze
- [ ] Phase C — TTY/private/global command, Article SQLite runtime, external project와 secret/process product flow
...
Activation 뒤 첫 구현은 Phase A artifact lock과 Phase B의 regression-first systemstate API split입니다. Q-019 terminal
quarantine과 two-app generalization probe는 서로 다른 후속/병렬 evidence lane이며 이 work의 public API나 global status를 동시에
수정하지 않습니다.
