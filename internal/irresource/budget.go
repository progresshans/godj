...
func (budget *Budget) ScanField(path string, field ir.Field) error {
	...
	if field.Default != nil {
		...
		for _, value := range []string{string(field.Default.Kind), field.Default.String, field.Default.Time, field.Default.Date, field.Default.DateTime} {
	...
	for _, choice := range field.Choices {
		for _, value := range []string{choice.Label, string(choice.Value.Kind), choice.Value.String, choice.Value.Time, choice.Value.Date, choice.Value.DateTime} {
