...
func ReadRequest(reader io.Reader) (Request, Failure, bool, error) {
	...
	document, err := wirejson.Read(reader, MaxRequestBytes, wirejson.StopOnOverflow)
	if err != nil {
...
func ParseResponse(document []byte, transportOK bool) (Response, Failure, bool) {
	...
	if err != nil {
		if errors.Is(err, wirejson.ErrResource) {
			return resourceResponseFailure()
		}
	...
	case "ok":
		...
		if !ok || !wirejson.HasKeys(resultObject, "statements") {
			return invalidResponse()
		}
		wireStatements, ok := resultObject["statements"].([]any)
		if !ok {
			return invalidResponse()
		}
...
func parseRequest(document []byte) (Request, Failure, bool) {
	...
	if !wirejson.HasKeys(object, "protocol_version", "command", "app", "name") || object["command"] != "migrations.sql" {
		return invalidRequest()
	}
...
func decodeObject(document []byte, maximum int) (map[string]any, error) {
	return wirejson.DecodeObject(document, wirejson.Limits{Bytes: maximum, ValueDepth: maxJSONDepth, Values: maxWireValues, ObjectKeys: maxWireObjectKeys, ArrayValues: maxWireArrayValues})
}
