...
func TestSQLiteMultipleEagerSelectionsMatchDjango(t *testing.T) {
	ctx := t.Context()
	...
	for _, statement := range []string{
		...
		`CREATE TABLE join_reference_team(id INTEGER PRIMARY KEY, label TEXT NOT NULL)`,
		`CREATE TABLE join_reference_post(id INTEGER PRIMARY KEY, title TEXT NOT NULL, author_id INTEGER NOT NULL REFERENCES join_reference_person(id), reviewer_id INTEGER NULL REFERENCES join_reference_person(id), team_id INTEGER NULL REFERENCES join_reference_team(id))`,
	...
	if _, err := backend.ExecContext(ctx, `INSERT INTO join_reference_team VALUES(1,'Red'),(2,'Blue')`); err != nil {
		t.Fatal(err)
	}
	for _, row := range [][]any{{1, "keep", 1, nil}, {2, "drop", 2, nil}, {3, "keep", 1, 1}, {4, "drop", 2, 1}, {5, "keep", 1, 2}, {6, "drop", 2, 2}, {7, "keep", 3, 3}} {
		...
		if _, err := backend.ExecContext(ctx, `INSERT INTO join_reference_post VALUES(?,?,?,?,?)`, append(row, team)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	data, err := os.ReadFile("../../orm/testdata/multiple-eager-django61.json")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, observation := range reference.Observations {
		t.Run(observation.Name, func(t *testing.T) {
			plan, err := nullableforwardproduct.MultipleEagerPlan(reference.Leaves, observation.MultipleEagerInput)
			if err != nil {
				t.Fatal(err)
			}
			before := backend.QueryCount()
			count, first, rows, err := nullableforwardproduct.EvaluateMultipleEager(ctx, backend, plan)
	...
	for name, plan := range querytest.ConflictingMultipleEagerPlans(t) {
