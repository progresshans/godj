...
func TestJSONOrderingCompilerBindsExactSelectedPathAndHiddenModelValues(t *testing.T) {
	...
	for _, distinct := range []bool{false, true} {
		for _, model := range []bool{false, true} {
			...
			var want []any
			path := sqliteJSONPathArgument(key)
			if model && !distinct {
				want = []any{int64(10), path, int64(3), int64(1)}
			} else if model || distinct {
			...
			if !strings.Contains(statement, "godj_json_sort_key(") {
				t.Fatal("path ordering lost exact numeric key", statement)
			}
