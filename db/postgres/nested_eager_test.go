...
func TestPostgreSQLNestedEagerGraphsMatchDjango(t *testing.T) {
	...
	schema := fmt.Sprintf("godj_nested_eager_%d", time.Now().UnixNano())
	...
	querytest.CheckNestedEagerReference(t, backend, func(plan query.Plan) (string, []any, error) { return compilePlan(schema, plan) }, func() (uint64, uint64) { return trace.reads.Load(), trace.total.Load() })
	querytest.CheckNestedEagerInvalidPlans(t, backend, func(plan query.Plan) (string, []any, error) { return compilePlan(schema, plan) }, func() (uint64, uint64) { return trace.reads.Load(), trace.total.Load() })
}
