...
func compileInsert(schema string, plan query.InsertPlan) (string, []any, error) {
	...
	columns, arguments, err := queryplan.Assignments(assignments, validateWriteValue, quoteIdentifier, nil, postgresValue)
	if err != nil {
		return "", nil, err
	}
...
func compileUpdate(schema string, plan query.UpdatePlan) (string, []any, error) {
	...
	keyColumn, keyArgument, err := queryplan.Key(plan.KeyField(), plan.KeyValue(), validateWriteValue, quoteIdentifier, postgresValue)
	if err != nil {
		return "", nil, err
	}
