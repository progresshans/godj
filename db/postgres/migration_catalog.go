...
func assertPostgresMigrationColumnCatalog(
	...
	case ir.FieldDecimal:
		if field.Decimal == nil || !field.Decimal.Valid() || actual.typeName != "numeric" || actual.typeModifier != ((field.Decimal.MaxDigits<<16)|field.Decimal.DecimalPlaces)+4 || actual.notNull == field.Nullable || actual.identity != "" {
			return fmt.Errorf("DecimalField column %q has an unsupported physical shape", field.Column)
		}
	case ir.FieldFloat:
		if actual.typeName != "float8" || actual.typeModifier != -1 || actual.notNull == field.Nullable || actual.identity != "" {
			return fmt.Errorf("FloatField column %q has an unsupported physical shape", field.Column)
		}
	case ir.FieldDuration:
		if actual.typeName != "interval" || actual.typeModifier != -1 || actual.notNull == field.Nullable || actual.identity != "" {
			return fmt.Errorf("DurationField column %q has an unsupported physical shape", field.Column)
		}
	case ir.FieldTime:
		if actual.typeName != "time" || actual.typeModifier != -1 || actual.notNull == field.Nullable || actual.identity != "" {
			return fmt.Errorf("TimeField column %q has an unsupported physical shape", field.Column)
		}
	case ir.FieldDate:
		if actual.typeName != "date" || actual.typeModifier != -1 || actual.notNull == field.Nullable || actual.identity != "" {
			return fmt.Errorf("DateField column %q has an unsupported physical shape", field.Column)
		}
	case ir.FieldDateTime:
		if actual.typeName != "timestamptz" || actual.typeModifier != -1 || actual.notNull == field.Nullable || actual.identity != "" {
			return fmt.Errorf("DateTimeField column %q has an unsupported physical shape", field.Column)
		}
	case ir.FieldText:
		if actual.typeName != "text" || actual.typeModifier != -1 || actual.notNull == field.Nullable || actual.identity != "" {
			return fmt.Errorf("TextField column %q has an unsupported physical shape", field.Column)
		}
