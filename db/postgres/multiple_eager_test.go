...
func TestPostgreSQLMultipleEagerSelectionsMatchDjango(t *testing.T) {
	...
	schema := fmt.Sprintf("godj_multiple_reference_%d", time.Now().UnixNano())
	...
	teamTable := quoted + `."join_reference_team"`
	...
	for _, statement := range []string{
		...
		`CREATE TABLE ` + teamTable + `(id BIGINT PRIMARY KEY, label TEXT NOT NULL)`,
		`CREATE TABLE ` + post + `(id BIGINT PRIMARY KEY, title TEXT NOT NULL, author_id BIGINT NOT NULL REFERENCES ` + author + `(id), reviewer_id BIGINT NULL REFERENCES ` + author + `(id), team_id BIGINT NULL REFERENCES ` + teamTable + `(id))`,
	...
	if _, err := admin.Exec(ctx, `INSERT INTO `+teamTable+` VALUES(1,'Red'),(2,'Blue')`); err != nil {
		t.Fatal(err)
	}
	for _, row := range [][]any{{int64(1), "keep", int64(1), nil}, {int64(2), "drop", int64(2), nil}, {int64(3), "keep", int64(1), int64(1)}, {int64(4), "drop", int64(2), int64(1)}, {int64(5), "keep", int64(1), int64(2)}, {int64(6), "drop", int64(2), int64(2)}, {int64(7), "keep", int64(3), int64(3)}} {
		...
		if row[3] == nil {
			team = int64(1)
		} else if row[3] == int64(2) {
			team = int64(2)
		}
		if _, err := admin.Exec(ctx, `INSERT INTO `+post+` VALUES($1,$2,$3,$4,$5)`, append(row, team)...); err != nil {
			t.Fatal(err)
		}
	}
