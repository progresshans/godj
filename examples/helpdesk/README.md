...
`POST /api/tickets/`는 subject/details/closed/priority를 받아 Ticket과 201을 반환하고 Location header를 추가하지 않는다.
...
하나의 JOIN 조회로 반환한다. 응답의 `ticket`은 id/subject/details/closed/category/priority, `category`는 id/name만 포함한다.
...
`0001_initial`은 보존하며 `0002_ticket_priority`가 nullable 정수를 추가한다. `MigrationSources()`의 전체 source를 loader에 전달한다.
테스트는 0001 schema에 먼저 행을 입력하고 0002 적용·reverse·재적용 뒤에도 기존 데이터와 null priority가 보존되는지 확인한다.
