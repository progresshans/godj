...
func (a *Application) API(authentication api.Authentication) (*API, error) {
	...
	create, err := protect(openapi.Operation{
		...
		RequestBody: &openapi.RequestBody{Schema: inputRef, Required: true, Description: fmt.Sprintf("The body must be one JSON object, limited to %d bytes and depth %d. The JSON string limit is %d bytes, subject to the smaller whole-body limit. Duplicate members and trailing data are rejected. Subject is required. Omitted closed defaults to false; omitted details, priority, resolution, due_at, reviewed, service_on and service_at become null. Service_at is a clock without a date or timezone, with microsecond precision; accepted ISO offsets are discarded without converting the clock. Service_on is a calendar date without a clock or timezone; ISO calendar, compact and week inputs normalize to YYYY-MM-DD in years 0001 through 9999. Reviewed accepts only JSON true, false, or null; false is distinct from null. Due_at requires an RFC 3339 timestamp with an explicit offset; instants are normalized to UTC microseconds, truncating finer precision. Empty details and resolution are distinct from null. Resolution is multiline text without a model length limit; the request body budget still applies. Priority accepts 1 (Urgent), 0 (Normal), -1 (Low), or null; zero is distinct from null. Existing stored priorities outside these input choices remain visible in responses. Subject, details and resolution text is trimmed. The generated id and assigned category cannot be supplied.", maximumJSONBodyBytes, serializers.DefaultMaxDepth, serializers.DefaultMaxStringBytes)},
