...
func (a *Application) API(authentication api.Authentication) (*API, error) {
	...
	detail, err := protect(openapi.Operation{
		...
		Description: "The identifier must be positive. The selected category is fixed by the application. Viewing a ticket includes its category identity and name; a separate category-view permission is not required. Authentication and permission checks precede the joined lookup. Labels are positive label identifiers in the selected category, sorted by identifier; viewing this owned relation does not require the standalone label-view permission.",
	...
	const updateDescription = "Authentication, CSRF when required, and change permission checks precede body parsing. A nonpositive identifier returns 404. Structural input validation precedes the target lookup. Uniqueness checks follow the authorized target lookup and exclude the current ticket. Non-null external references are unique across all tickets; SQL null references are distinct. Duplicate input returns 400 validation_error with external_reference/unique; a concurrent storage conflict uses __all__/unique and rolls back the update. The target must belong to the application's selected category. The current row lookup and update share one transaction. Generated id and assigned category cannot be supplied. ChangeTicket and ViewLabel are required before parsing. Omitted labels preserve membership in both PUT and PATCH; an explicit empty array clears it. Null and non-integer members are rejected. Positive labels must all belong to the selected category. The complete desired set and admitted permissions are rechecked in the scalar write transaction. Retained links keep their identities. Omitted nullable fields preserve their stored values; explicit null clears them. Reviewed accepts only JSON true, false, or null. Service_on accepts ISO calendar, compact and week dates and returns YYYY-MM-DD without a clock or timezone. Service_at returns HH:MM:SS with six fractional digits when nonzero; omitted values are preserved and null clears the clock."
